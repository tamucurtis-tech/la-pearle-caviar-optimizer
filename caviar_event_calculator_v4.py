...
# Caviar Event Calculator — v4
# Key changes:
# - New objective: "Fewest tins (recommended)" prioritizes minimal tin count, then cost, then overage.
# - Alternatives panel now shows Top N mixes with FEWEST tins (not % of best cost).
# - Optional 125 g share cap limits reliance on 125 g tins.
# - Keeps PDF Summary Sheet, branding, and 1h/2h/3h outputs from v3.
...
top_k = st.sidebar.number_input("Show Top N mixes (fewest tins)", min_value=1, value=10, step=1)
...
def optimize_and_rank(req_g,
        ...
        for x8 in range(max_8 + 1):
            ...
            for x125 in range(max_125 + 1):
                ...
                if objective == "Cheapest only":
                    ...
                    # tins first, then cost, then overage
    ...
    # alternatives: top N by FEWEST TINS, breaking by cost then overage
    alt_sorted = sorted(combos, key=lambda t: (t[1], t[2], t[3]))
    alts = []
    for t in alt_sorted:
        mix = {
            "x250": t[4], "x8": t[5], "x7": t[6], "x125": t[7],
            "purchased_g": t[8], "overage_g": t[3],
            "total_tins": t[1], "total_cost": t[2]
        }
        if mix not in alts:
            alts.append(mix)
        if len(alts) >= top_k:
...
st.caption("Fewest-tins mode favors larger tins (250 g / 8 oz). Use the 125 g share cap to avoid all-125 solutions. Alternatives list the top mixes with the FEWEST total tins.")
