...
# Caviar Event Calculator — v4.1 (formatting & clarity update)
# - Top Mixes "Total cost ($)" column is rounded UP to whole dollars (no decimals).
# - "Optimal mix" row is bold in each results table.
# - Sidebar label changed to "Only Show Top ___ Mixes".
# - Clear definition for "Per‑tin service penalty" in Mix Strategy.
...
def optimize_and_rank(req_g,
    ...
    for t in alt_sorted:
        ...
        alts.append({
            ...
            "Total tins": t[1], "Total cost ($)": round(t[2], 2)
        })
        if len(alts) >= top_k:
            break
...
def render_block(title, best):
    ...
    # Bold the "Optimal mix" row using Styler
    def _bold_optimal(row):
        return ['font-weight: bold' if row['Metric'] == 'Optimal mix' else '' for _ in row]
    styled = df.style.apply(_bold_optimal, axis=1).hide_index()
...
def render_alternatives(title, alts, label="Top mixes (fewest tins first)"):
    if alts:
        ...
        # Column H is "Total cost ($)" — round UP to whole dollars
        if "Total cost ($)" in df2.columns:
            df2["Total cost ($)"] = df2["Total cost ($)"].apply(lambda x: int(math.ceil(float(x))))
...
st.caption("v4.1: Bold Optimal mix row; Top Mixes show whole‑dollar costs (rounded up); clearer Mix Strategy help; label reads 'Only Show Top ___ Mixes'.")
