...
# ---------------- Sidebar: Brand + Advanced Options ----------------
...
cap_250 = st.sidebar.number_input("Max 250 g tins (optional)", min_value=0, value=0, step=1, help="0 means no cap")
cap_8   = st.sidebar.number_input("Max 8 oz tins (optional)",   min_value=0, value=0, step=1, help="0 means no cap")
cap_125 = st.sidebar.number_input("Max 125 g tins (optional)",  min_value=0, value=0, step=1, help="0 means no cap")
alt_within = st.sidebar.slider("Show alternatives within % of best cost", 0, 25, 5, help="0 = show only the best. Increase to see near-optimal mixes.")
...
st.write("Compute the lowest-cost **mix** of tins that meets your required grams for **one-hour**, **two-hour**, and **three-hour** events.")
...
def optimize_and_rank(req_g,
                      ...
                      alt_within_pct=0, top_k=10):
    # derive bounds (caps override if provided)
    ...
    for x250 in range(max_250 + 1):
        ...
        for x8 in range(max_8 + 1):
            ...
            for x125 in range(max_125 + 1):
                ...
                combos.append((cost, over, total_tins, x250, x8, x7, x125, purchased))

    combos.sort(key=lambda t: (t[0], t[1], t[2]))  # cost, then overage, then fewest tins
    best = combos[0]
    res_best = {
        "required_g": req_g,
    ...
    # Alternatives within % of best cost (distinct mixes)
    alt = []
    limit_cost = best[0] * (1 + alt_within_pct/100.0)
    for t in combos[1:]:
        if t[0] <= limit_cost:
            alt.append({"x250": t[3], "x8": t[4], "x7": t[5], "x125": t[6],
                        "purchased_g": t[7], "overage_g": t[1],
                        "total_tins": t[2], "total_cost": t[0]})
            if len(alt) >= top_k:
                break
...
def render_block(title, best):
    ...
    df = pd.DataFrame({
        ...
        "Value": [
            ...
            f"${best['total_cost']/guests:,.2f}",
        ]
    })
    st.dataframe(df, use_container_width=True, hide_index=True)

def render_alternatives(title, alts, within_pct):
    if alts:
        st.markdown(f"**Alternative mixes within {within_pct}% of best cost**")
        df2 = pd.DataFrame([{
            "250 g": a["x250"], "8 oz": a["x8"], "7 oz": a["x7"], "125 g": a["x125"],
            "Total grams": a["purchased_g"], "Overage (g)": a["overage_g"],
            "Total tins": a["total_tins"], "Total cost ($)": round(a["total_cost"], 2)
        } for a in alts])
        st.dataframe(df2, use_container_width=True, hide_index=True)

# Required grams for each event
...
# Run optimizer with caps and alternatives
best_1h, alt_1h = optimize_and_rank(req_1h,
    grams_250, price_250, grams_8, price_8, grams_7, price_7, grams_125, price_125,
    cap_250=cap_250, cap_8=cap_8, cap_125=cap_125, alt_within_pct=alt_within, top_k=10)

best_2h, alt_2h = optimize_and_rank(req_2h,
    grams_250, price_250, grams_8, price_8, grams_7, price_7, grams_125, price_125,
    cap_250=cap_250, cap_8=cap_8, cap_125=cap_125, alt_within_pct=alt_within, top_k=10)

best_3h, alt_3h = optimize_and_rank(req_3h,
    grams_250, price_250, grams_8, price_8, grams_7, price_7, grams_125, price_125,
    cap_250=cap_250, cap_8=cap_8, cap_125=cap_125, alt_within_pct=alt_within, top_k=10)
...
render_alternatives("One-Hour Event", alt_1h, alt_within)

render_block("Two-Hour Event", best_2h)
render_alternatives("Two-Hour Event", alt_2h, alt_within)

render_block("Three-Hour Event", best_3h)
render_alternatives("Three-Hour Event", alt_3h, alt_within)
...
def generate_pdf(brand_name, logo_path_or_url, bg_hex, ink_hex, gold_hex,
    ...
    y = height - 72  # 1" margin top

    # Logo (if available)
    if logo_path_or_url:
        try:
            img = ImageReader(logo_path_or_url)
            # Fit logo within 2.5" width
            logo_w = 2.5 * inch
    ...
    c.setFillColor(gold)
    c.drawCentredString(width/2, y, "Caviar Event Summary")
    y -= 18 + 6
    c.setFont("Helvetica", 10)
    c.setFillColor(ink)
    c.drawCentredString(width/2, y, f"{today}")
    y -= 22

    # Brand attribution
    c.setFont("Helvetica", 9)
    c.drawCentredString(width/2, y, f"Prepared by {brand_name}")
    y -= 18
    ...
    c.drawString(72, y, "Event Inputs")
    y -= 14
    c.setFont("Helvetica", 10)
    c.drawString(72, y, f"Guests: {guests}")
    y -= 12
    c.drawString(72, y, f"Grams per tasting: {gpt}")
    y -= 12
    c.drawString(72, y, f"Tastings per guest (1h/2h/3h): {t1} / {t2} / {t3}")
    y -= 18

    # Helper to render a result block
    def block(title, best):
        ...
        # draw light panel
        c.setFillColor(bg)
        c.setStrokeColor(colors.Color(0,0,0,0.05))
...
if not REPORTLAB_AVAILABLE:
    ...
    # Pick a logo source for the PDF (local path preferred)
...
st.caption("Summary sheet is portrait (US Letter). Title includes the current date. Costs only (no profit line). Add 'reportlab' to requirements.txt to enable the PDF download.")
